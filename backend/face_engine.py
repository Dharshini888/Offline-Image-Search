...
#         # Normal cosine dist threshold is 1 - Cosine Sim. 
#         # A threshold of 0.4 means Cosine Similarity must be 0.60 or higher
#         model = DBSCAN(eps=0.40, min_samples=2, metric="cosine")
...
class FaceEngine:
    ...
    def cluster_faces(self, embeddings: list):
        ...
        norms = np.linalg.norm(matrix, axis=1, keepdims=True)
        matrix = matrix / (norms + 1e-10)
        model = DBSCAN(eps=0.50, min_samples=1, metric="cosine")
        labels = model.fit_predict(matrix)
        return labels
